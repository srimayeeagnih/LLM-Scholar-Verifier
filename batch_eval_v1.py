...
# Import pipeline functions from MCP_test (must be in the same folder)
PROJECT_DIR = os.path.dirname(os.path.abspath(__file__))
sys.path.insert(0, PROJECT_DIR)
from MCP_test import search_by_claims, compute_claim_verdict


COLUMNS = [
    "row_index", "question", "claim_text",
    "papers_found", "agree", "disagree", "missing",
    "validity_score", "agreement_rate", "disagreement_rate",
    "coverage_rate", "consistency_rate",
]
            ...
            for local_i, (abs_i, row) in enumerate(rows_in_batch):
                ...
                try:
                    ...
                    for claim_data in claim_results:
                        ...
                        total_missing  += ct.get("missing", 0)
                        total_papers   += len(
                            claim_data.get("results", {}).get("combined", [])
                        )
                        agreement_rates.append(ct.get("agreement_rate", 0.0))
                        disagreement_rates.append(ct.get("disagreement_rate", 0.0))
                        # coverage_rate already reflects TF-IDF keyword weights from MCP_test.py
                        coverage_rates.append(ct.get("coverage_rate", 0.0))
                        consistency_rates.append(ct.get("consistency_rate", 0.0))

                    # Average each component rate across sub-claims, then apply the formula:
                    # validity_score = agreement_rate × coverage_rate × consistency_rate
                    n = len(agreement_rates) if agreement_rates else 1
                    agreement_rate    = sum(agreement_rates)    / n
                    disagreement_rate = sum(disagreement_rates) / n
                    coverage_rate     = sum(coverage_rates)     / n
                    consistency_rate  = sum(consistency_rates)  / n
                    validity_score    = agreement_rate * coverage_rate * consistency_rate
                    ...
                    print(f"\n  Validity score: {validity_score:.1%}  "
                          ...
                          f"missing={total_missing}  papers={total_papers})")
                    print(f"  Agreement: {agreement_rate:.1%}  "
                          f"Coverage (TF-IDF weighted): {coverage_rate:.1%}  "
