...
# ──────────────────────────────────────────────
# Word2Vec Skip-gram Context Analysis
# ──────────────────────────────────────────────
def train_w2v_skipgram(papers, vector_size=100, window=5, epochs=20):
    """Train a Word2Vec Skip-gram model on sentences from all papers.

    Args:
        papers:      List of paper dicts (must have 'full_text' or 'abstract').
        vector_size: Dimensionality of word embeddings.
        window:      Context window size (tokens before/after center word).
        epochs:      Training epochs.

    Returns:
        Trained Word2Vec model, or None if no sentences found.
    """
    all_sentences_tokenized = []
    for paper in papers:
        text = paper.get('full_text') or paper.get('abstract') or ''
        for sent in extract_sentences(text):
            tokens = re.findall(r'\b[a-zA-Z][a-zA-Z0-9]*\b', sent.lower())
            if tokens:
                all_sentences_tokenized.append(tokens)

    if not all_sentences_tokenized:
        return None
    ...
    print(f"  Word2Vec Skip-gram trained: {len(model.wv)} words, "
          f"vector_size={vector_size}, window={window}, epochs={epochs}")
    return model


def compute_context_vector(tokens, keyword_positions, window, w2v_model):
    """Average the Word2Vec embeddings of context words around keyword positions.

    Args:
        tokens:            List of lowercase word tokens.
        keyword_positions: List of int positions where the keyword appears.
        window:            Number of tokens before/after to include.
        w2v_model:         Trained Word2Vec model.

    Returns:
        numpy array (averaged context vector), or None if no context words found.
    """
    import numpy as np
        ...
        for offset in range(-window, window + 1):
            ...
            if 0 <= idx < len(tokens):
                ...
                if word in w2v_model.wv:
                    context_vecs.append(w2v_model.wv[word])
...
def w2v_context_similarity(claim_text, sentence_text, keyword, w2v_model,
                           window=5, synonym_group=None):
    """Compute Skip-gram context similarity for a keyword between claim and sentence.

    Finds all positions of the keyword (or its synonyms) in both texts,
    builds averaged context vectors, and returns their cosine similarity.

    Args:
        claim_text:    The user's claim string.
        sentence_text: A sentence from a paper.
        keyword:       The keyword to compare context around.
        w2v_model:     Trained Word2Vec model.
        window:        Context window size.
        synonym_group: Optional set of synonyms for the keyword.

    Returns:
        Float cosine similarity (0.0–1.0), or None if keyword not found in both.
    """
    import numpy as np

    def find_keyword_positions(tokens, kw, syn_group):
        """Find positions where keyword or its synonyms appear."""
        kw_stem = simple_stem(kw)
        positions = []
        for i, tok in enumerate(tokens):
            if simple_stem(tok) == kw_stem or tok == kw:
                positions.append(i)
            elif syn_group:
                for syn in syn_group:
                    if ' ' not in syn and simple_stem(tok) == simple_stem(syn):
                        positions.append(i)
                        break
        return positions

    claim_tokens = re.findall(r'\b[a-zA-Z][a-zA-Z0-9]*\b', claim_text.lower())
    sent_tokens = re.findall(r'\b[a-zA-Z][a-zA-Z0-9]*\b', sentence_text.lower())

    claim_positions = find_keyword_positions(claim_tokens, keyword, synonym_group)
    sent_positions = find_keyword_positions(sent_tokens, keyword, synonym_group)

    if not claim_positions or not sent_positions:
        return None

    claim_ctx = compute_context_vector(claim_tokens, claim_positions, window, w2v_model)
    sent_ctx = compute_context_vector(sent_tokens, sent_positions, window, w2v_model)

    if claim_ctx is None or sent_ctx is None:
        return None

    dot = np.dot(claim_ctx, sent_ctx)
    norm1 = np.linalg.norm(claim_ctx)
    norm2 = np.linalg.norm(sent_ctx)
...
def compare_claim_to_sentences(proximity_map, sentences, keywords, window=5,
                               keyword_synonym_groups=None, w2v_model=None,
                               claim_text=None):
    ...
    When a Word2Vec model is provided, also computes Skip-gram context similarity
    around each keyword and produces a fused_agreement score:
      - If an explicit negator is found on either side, the negator flag dominates.
      - Otherwise, the context similarity score is used (catches soft negation).
    ...
    Args:
        ...
        w2v_model:     Optional trained Word2Vec Skip-gram model. When provided,
                       context similarity is computed and fused with negator flags.
        claim_text:    The user's claim string (required when w2v_model is provided).

    Returns:
        List of dicts, one per sentence, each with 'keyword_comparisons' containing:
            agreement:          bool  (negator-based, original)
            context_similarity: float or None (Skip-gram context cosine, new)
            fused_agreement:    float 0.0–1.0 (combined score, new)
    """
    ...
    for sent_text, sim_score in sentences:
        ...
        for i, kw in enumerate(keywords):
            ...
            negator_agreement = claim_negated == sent_negated

            # ── Skip-gram context similarity ──
            ctx_sim = None
            if w2v_model is not None and claim_text is not None and in_sent:
                syn_group = synonym_map.get(kw) if synonym_map else None
                ctx_sim = w2v_context_similarity(
                    claim_text, sent_text, kw, w2v_model,
                    window=window, synonym_group=syn_group
                )

            # ── Fused agreement: negator flag dominates when present ──
            has_explicit_negator = claim_negated or sent_negated
            if has_explicit_negator:
                # Hard negator found — trust the boolean flag
                fused = 1.0 if negator_agreement else 0.0
            elif ctx_sim is not None:
                # No explicit negator — use context similarity to detect soft negation
                fused = ctx_sim
            else:
                # No negator, no context signal — default to agreement
                fused = 1.0 if negator_agreement else 0.0
            ...
            comparisons.append({
                ...
                'agreement': negator_agreement,
                'context_similarity': ctx_sim,
                'fused_agreement': fused,
...
def build_bow(text, min_length=3):
    """Build a Bag of Words (stemmed word frequencies) from text.

    Captures both regular words and measurement tokens like '100 degrees celsius'.
    """
    stopwords = {'the', 'and', 'for', 'that', 'this', 'with', 'from', 'are', 'was',
                 'were', 'been', 'have', 'has', 'will', 'would', 'could', 'should',
                 'about', 'into', 'through', 'during', 'before', 'after', 'above',
    ...
    # Pass 1: Extract measurement tokens (number + unit), kept as-is (no stemming)
    measurement_pattern = re.compile(
        r'\b(\d+(?:\.\d+)?)\s*'
        r'(degrees?\s*(?:celsius|fahrenheit|celcius|fareinheit|kelvin|[cfk])'
        r'|°[cfk]'
        r'|%|percent'
        r'|m[lg]|kg|[gc]m|mm|nm|µm|µg|µl'
        r'|mol(?:/l)?|mmol|µmol'
    ...
    measurements = [f"{m.group(1)} {m.group(2).strip()}" for m in measurement_pattern.finditer(lowered)]

    # Pass 2: Regular word tokens (alphabetic + alphanumeric like 'p53', 'covid19')
    words = re.findall(r'\b[a-zA-Z][a-zA-Z0-9]*\b', lowered)
    stemmed = [simple_stem(w) for w in words if len(w) >= min_length and w not in stopwords]

    return Counter(stemmed + measurements)


def cosine_similarity_bow(bow1, bow2):
    """Compute cosine similarity between two BoW Counters."""
    all_keys = set(bow1) | set(bow2)
    if not all_keys:
        return 0.0
    dot = sum(bow1.get(k, 0) * bow2.get(k, 0) for k in all_keys)
...
def rank_sentences_by_claim(paper_text, claim_bow, top_n=5):
    """Rank sentences from a paper by cosine similarity to the claim BoW.

    Returns list of (sentence, similarity_score) sorted descending.
    """
    sentences = extract_sentences(paper_text)
    scored = []
    for sent in sentences:
        sent_bow = build_bow(sent)
        score = cosine_similarity_bow(claim_bow, sent_bow)
        if score > 0:
...
def search_all_databases(user_input, max_results_per_source=100):
    ...
    # ── Stage 4a: Train Word2Vec Skip-gram for context analysis ──
    print(f"\n--- Stage 4a: Training Word2Vec Skip-gram ---")
    w2v_model = train_w2v_skipgram(combined, vector_size=100, window=5, epochs=20)

    # ── Stage 4b: Claim word retrieval & negation analysis (fused with W2V) ──
    print(f"\n--- Stage 4b: Negation analysis + Skip-gram context ---")
    ...
    for paper in combined:
        ...
        for comp in comparisons:
            for kc in comp['keyword_comparisons']:
                if kc['in_sentence'] and kc['fused_agreement'] < 0.5:
                    ctx_str = (f" ctx_sim={kc['context_similarity']:.2f}"
                               if kc['context_similarity'] is not None else "")
                    print(f"  {paper['title'][:50]}...")
                    print(f"    [DISAGREE] keyword '{kc['keyword']}': "
                          f"claim_negated={kc['claim_negated']}, "
                          f"sentence_negated={kc['sentence_negated']}"
                          f"{ctx_str} fused={kc['fused_agreement']:.2f}")
...
def compute_negation_scores(paper):
    ...
    for comp in paper.get('sentence_comparisons', []):
        for kc in comp['keyword_comparisons']:
            ...
            # Use fused_agreement (float) when available, fall back to boolean
            fused = kc.get('fused_agreement')
    ...
    # Per-keyword coverage: how many unique keywords appeared in at least 1 sentence
